...
clear
echo "=============================="
echo "    Gost 配置预览 (全解析模式)"
echo "=============================="

if [ -s "$CONF_FILE" ]; then
    # 深度递归逻辑：
    # 1. 找到所有包含 addr 的对象
    # 2. 如果 addr 以 ":" 开头，判定为“监听端口”
    # 3. 如果 addr 包含 "." 且带端口，判定为“落地节点”
    # 4. 自动关联它们所属的 service 块
    DATA=$(jq -r '
        .services[]? | . as $svc | 
        $svc.addr as $port | 
        ($svc | .. | .addr? | select(. != null and contains(".") and contains(":"))) as $node |
        "\($port) \t \($node)"
    ' "$CONF_FILE" 2>/dev/null)

    if [ -z "$DATA" ]; then
        echo "💡 提示：未能通过深度扫描识别映射。请选 4 手动确认。"
    else
